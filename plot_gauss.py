...
class plot_gauss:
    ...
    def __init__(self,imode):
        ...
        self.output      = Checkbox( value=True , description='1D:')
        self.np          = BoundedIntText( value=1000, min=100,   max=3000,  step=100, description='n grid points (x,y,z directions):')
        self.xpos        = BoundedIntText( value=3000, min=1,     max=11000, step=1,   description='x-position (m):')
        self.ypos        = BoundedIntText( value=0,    min=-2700, max=2700,  step=1,   description='y-position (m):')
        self.zpos        = BoundedIntText( value=50,   min=0,     max=3200,  step=1,   description='z-position (m):')
        self.dxpos       = BoundedIntText( value=10,   min=1,     max=100,   step=1,   description='dx (m):')
        self.dypos       = BoundedIntText( value=10,   min=1,     max=100,   step=1,   description='dy (m):')
        self.dzpos       = BoundedIntText( value=10,   min=1,     max=100,   step=1,   description='dz (m):')
        ...
        if imode == 3: self.type.value='line+reflection'
        ...
        # --- Form 1: Upper box with GPM settings    
        form_items       = [
                           ...
                           #Box([self.stability,self.conc ],                                            layout = form_item_layout),
                            Box([self.stability ],                                                      layout = form_item_layout),
        ...
        # --- Form 2: lower box with display settings and concentration
        form_items2      =  [
...
#                           Box([self.stability,self.conc ], layout = form_item_layout)
                            Box([self.conc            ], layout = form_item_layout)
                            ]
        ...
        self.set_visibility(imode) # MKvdM : why??
        output_notebook()
    ...
    def gauss(self):
       ...
       # print("At %.i m" %(self.dist.value), "sigma_y: %.f m" %(sigma_y[dist_ind]), " sigma_z: %.f m" %(sigma_z[dist_ind]))
...
# ---   yz cross-section at x
        ...
        if stype == 'line+reflection':
            print("At x = %.i m:" %(self.xpos.value), "sigma_z = %.f m" %(sigma_z))
        else:
            print("At x = %.i m:" %(self.xpos.value), "sigma_y = %.f m" %(sigma_y), " sigma_z = %.f m" %(sigma_z))
